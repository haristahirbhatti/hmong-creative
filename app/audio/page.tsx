    ...
    <div style={{ background: '#141414', borderRadius: 16, border: '1px solid rgba(255,255,255,0.07)', padding: '16px', marginBottom: 12, flexShrink: 0 }}>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          ...
          <div style={{ flex: 1, height: 3, background: '#2a2a2a', borderRadius: 100, cursor: 'pointer', position: 'relative' }}
            onClick={e => { if (!audioRef.current) return; const rect = e.currentTarget.getBoundingClientRect(); audioRef.current.currentTime = ((e.clientX - rect.left) / rect.width) * duration; }}>
      ...
      <audio ref={audioRef} src={clip.audioUrl}
        ...
        onLoadedMetadata={() => { setDuration(audioRef.current?.duration || 0); if (index === 0) { audioRef.current?.play(); setIsPlaying(true); } }}
...
function AudioContent() {
  ...
  const handleGenerateWithArgs = async (args?: { title: string; lyrics: string; selectedTags: string[]; instrumental: boolean; }) => {
    ...
    const fullStyle = [
      ...
      tags.filter(tag => TAGS.Voice.includes(tag)).join(', '),
    ...
    try {
      const res = await fetch('/api/audio', {
        ...
        body: JSON.stringify({ prompt: t, lyrics: instr ? '' : l, style: fullStyle, instrumental: instr, userId, userEmail, title: t }),
      });
  ...
  const handleEditClip = (clip: GeneratedClip) => {
    setTitle(clip.title); setLyrics(clip.lyrics);
    setTags([...clip.selectedTags]); setInstrumental(clip.instrumental);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };

  const handleRegenerateClip = (clip: GeneratedClip) => {
    setTitle(clip.title); setLyrics(clip.lyrics);
    setTags([...clip.selectedTags]); setInstrumental(clip.instrumental);
      ...
      <div style={{ display: 'flex', flex: 1, marginTop: 56 }}>
        ...
        <main style={{ marginLeft: 200, marginRight: hasSongs ? 400 : 0, flex: 1, padding: '24px 28px 40px', minWidth: 0, maxWidth: 780, transition: 'margin-right 0.3s ease' }}>
          ...
          <div style={{ marginBottom: 24, background: '#0f0f0f', borderRadius: 14, border: '1px solid rgba(255,255,255,0.06)', padding: '16px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {AI_TOOLS.map(tool => {
                const isLocked = tool.needsSong && !hasSongs;
                const isActive = activeToolId === tool.id;
                return (
                  <button key={tool.id}
                    onClick={() => { handleToolClick(tool.id); if (!isLocked && activeToolId !== tool.id) runTool(tool.id); }}
                    style={{ padding: '8px 14px', borderRadius: 100, border: `1px solid ${isActive ? '#FF5C2B' : isLocked ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.1)'}`, background: isActive ? 'rgba(255,92,43,0.15)' : 'transparent', color: isActive ? '#FF5C2B' : isLocked ? '#333' : '#777', fontSize: 13, fontWeight: isActive ? 700 : 400, cursor: isLocked ? 'not-allowed' : 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6, transition: 'all 0.15s' }}>
                    {tool.icon} {tool.label}
                    {isLocked && <span style={{ fontSize: 10, opacity: 0.5 }}>🔒</span>}
                  </button>
                );
              })}
            </div>
              ...
              <div style={{ marginTop: 14, background: '#141414', borderRadius: 12, padding: '14px', border: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                {activeToolId === 'gen-lyrics' && (
                  <div>
                    ...
                    {!toolLyrics && !toolLoading && (
                      <button onClick={() => runTool('gen-lyrics')} style={{ padding: '10px 20px', borderRadius: 10, border: 'none', background: '#FF5C2B', color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Generate Lyrics</button>
                    )}
                    {toolLyrics && <>
                      <pre style={{ fontSize: 13, color: '#aaa', whiteSpace: 'pre-wrap', lineHeight: 1.8, margin: '0 0 10px', fontFamily: 'inherit' }}>{toolLyrics}</pre>
                ...
                {activeToolId === 'cover-gen' && (
                  <div>
                    ...
                    <div style={{ marginBottom: 12 }}>
                      ...
                      <input value={toolCoverStyle} onChange={e => setToolCoverStyle(e.target.value)} placeholder="e.g. Hmong folk, pop..."
                        style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.08)', background: '#1a1a1a', color: 'white', fontSize: 13, outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }} />
                    </div>
                    {toolCoverResult && (
                      <div style={{ marginBottom: 12 }}>
                    ...
                    <button disabled={!toolCoverFile || toolLoading}
                      ...
                      {toolLoading ? <><span style={{ width: 12, height: 12, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 0.8s linear infinite', display: 'inline-block' }} />Generating...</> : '🔁 Generate Cover'}
                    </button>
                  </div>
                )}
          ...
          {error && <div style={{ background: 'rgba(255,68,68,0.08)', border: '1px solid rgba(255,68,68,0.25)', borderRadius: 12, padding: '12px 16px', marginBottom: 16, color: '#ff6666', fontSize: 13, lineHeight: 1.5 }}>⚠️ {error}</div>}
        ...
        {hasSongs && (
          <aside style={{ width: 390, position: 'fixed', top: 56, right: 0, bottom: 0, background: '#0d0d0d', borderLeft: '1px solid rgba(255,255,255,0.08)', padding: '24px 20px', overflowY: 'auto', zIndex: 100, display: 'flex', flexDirection: 'column' }}>
            ...
            <button onClick={() => { setClips([]); setStatus('idle'); }} style={{ marginTop: 8, width: '100%', padding: '10px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', background: 'transparent', color: '#333', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
              Clear &amp; Start Fresh
            </button>
          </aside>
        )}
